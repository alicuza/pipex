...
/*   Updated: 2026/03/16 15:56:26 by sancuta          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t	get_start_idx(t_env *env, char **argv)
{
	size_t start_idx;
...
void	parse_to_nodes(t_env *env, char **argv)
{
	...
	start_idx = get_start_idx(env, argv);
	env->node[0].data_idx = get_cmdv(arena,
	...
	while (++i < env->node_cnt - 1)
	{
		...
		env->node[i].data_idx = get_cmdv(arena,
				arena_split(env->data, argv[start_idx], WORD_DEL),
				count_words(argv[start_idx], WORD_DEL));
	}
