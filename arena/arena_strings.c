...
size_t	arena_split(t_arena *dest, const char *src, char del)
{
	...
	while (i < word_cnt)
	{
		...
		((char **)(dest->buf + offset))[i] = dest->buf + arena_strlcpy(dest, src, (len + 1) * sizeof(char)); // think about returning the offset & rewriting the get_cmdv function
		src += len;
		i++;
	}
	((char **)(dest->buf + offset))[i] = NULL; // NULL ptr as terminator
	return (offset);
}
